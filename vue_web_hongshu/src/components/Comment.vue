<template>
  <div class="comments-container">
    <div class="total">{{ $t("note.totalComments", { n: computedTotal }) }}</div>
    <div class="list-container">
      <div class="parent-comment" v-for="(oneComment, oneIndex) in dataList" :key="oneIndex">
        ...
        <div class="reply-container">
            ...
            <div class="comment-item" v-for="(twoComment, twoIndex) in oneComment.children" :key="twoIndex">
              <div class="comment-inner-container">
                ...
                <div class="right">
                  ...
                  <div class="content">
                    {{ $t("note.reply")
                    }}<span style="color: rgba(61, 61, 61, 0.8)">{{ twoComment.replyUsername }}: </span
                    >{{ twoComment.content }}
                  </div>
                  ...
                  <div class="info">
                    ...
                    <div class="interactions">
                      ...
                      <div class="reply" @click="saveComment(twoComment, oneIndex, twoIndex)">
                        <span class="like-wrapper">
                          <ChatRound style="width: 1.2em; height: 1.2em" />
                          <span class="count">{{ $t("note.reply") }}</span>
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          >
            {{ $t("note.expandReplies", { n: expandReplyRemaining(oneComment) }) }}
          </div>
          <div
            class="show-more"
            v-if="
...
import { getCommentWithCommentByNoteId, getCommentWithCommentByProductId, getTwoCommentPageByOneCommentId } from "@/api/comment";
...
const expandReplyRemaining = (c: any) => {
  if (import.meta.env.DEV && false) {
    t("note.reply");
  }
  const shown = c.children?.length ?? 0;
  const total = c.twoCommentCount ?? 0;
  return Math.max(1, total - shown);
};
...
const getCommentData = () => {
  ...
  // 根据 nid 或 productId 判断调用不同的接口
  const apiCall = props.nid 
    ? getCommentWithCommentByNoteId(props.currentPage, pageSize, props.nid)
    : getCommentWithCommentByProductId(props.currentPage, pageSize, props.productId);
...
watch(
  () => [props.nid, props.productId, props.seed, props.currentPage],
  ([newNid, newProductId, newSeed], [oldNid, oldProductId, oldSeed]) => {
    if (newNid !== oldNid || newProductId !== oldProductId) {
      dataList.value = [];
      getCommentData();
    }
    if (newSeed !== oldSeed) {

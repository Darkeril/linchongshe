<template>
  <div class="sug-container-wrapper query-trending sug-pad">
    <div class="sug-container query-trending">
      ...
      <div class="sug-box">
        <div class="header">{{ $t("search.suggestions") }}</div>
        <div class="sug-wrapper">
          <div class="sug-item query-trending hotspot" v-for="(item, index) in recommendRecords" :key="index">
...
import type { EsRecordDTO } from "@/type/note";
...
const recommendRecords = ref<Array<string>>([]);
...
const esRecordDTO = ref<EsRecordDTO>({
  keyword: "",
  uid: "",
});
...
const fetchHotTags = async () => {
  try {
    const res = (await getHotTagList(1, 10)) as any;
    if (res.code === 200 && res.data && res.data.records) {
      ...
      recommendRecords.value = tags;
    }
  } catch (error) {
    console.error("获取热门标签失败:", error);
    const fallback = tm("search.defaultTags") as string[];
    recommendRecords.value = Array.isArray(fallback) ? [...fallback] : [];
  }
};
...
const searchPage = (keyword: string) => {
  esRecordDTO.value.uid = userStore.getUserInfo().id;
  esRecordDTO.value.keyword = keyword;
  addRecord(esRecordDTO.value);
  // addRecord(keyword);
...
.sug-container-wrapper {
  ...
  box-shadow: 0 0.25rem 2rem 0 rgba(0, 0, 0, 0.08), 0 0.0625rem 0.25rem 0 rgba(0, 0, 0, 0.04);

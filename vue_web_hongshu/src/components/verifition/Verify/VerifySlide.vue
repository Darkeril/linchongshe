    ...
    export default {
        ...
        setup(props,context){
            ...
            let secretKey = ref(''),           //后端返回的ase加密秘钥
                ...
                function init() {
                    text.value = explain.value || t('verify.slideRight')
                    getPictrue();
                    nextTick(() => {
                    ...
                    if (status.value && isEnd.value == false) {
                        ...
                        reqCheck(data).then(res=>{
                            if (res.repCode == "0000") {
                                ...
                                passFlag.value = true
                                tipWords.value = `${((endMovetime.value-startMoveTime.value)/1000).toFixed(2)}s ${t('verify.success')}`
                                ...
                                tipWords.value = t('verify.failed')
                                setTimeout(()=>{
                                        tipWords.value = ""
                                },1000)
                            }

...
<template>
  <div
    ref="messageRef" class="flex w-full p-5 overflow-hidden"
  ...
  >
    ...
    <div
      class="items-start w-full overflow-hidden"
    ...
    >
      <div class="flex gap-1 pl-1" :class="[inversion ? 'flex-row' : 'flex-row']">
        <TextComponent
          v-if="contentType === 'text'" ref="textRef" :inversion="inversion" :error="error" :text="text" :loading="loading"
          :as-raw-text="asRawText"
        />
        <ImageComponent
          v-else ref="textRef" :inversion="inversion" :error="error" :images="images" :loading="loading"
          :as-raw-text="asRawText"
        />
      </div>
      <div v-if="!inversion && !loading" class="mt-4 h-[33px] w-full flex items-center justify-between">
        <div class="text-[#B0B7C0] text-xs">
          {{ t('chat.messageTip') }}
        </div>
        <div v-if="contentType === 'text'" class="flex items-center">
          ...
          <NDropdown
            :trigger="isMobile ? 'click' : 'hover'" :placement="!inversion ? 'right' : 'right'"

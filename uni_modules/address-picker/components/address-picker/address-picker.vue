...
<script>
	...
	export default {
		...
		watch: {
			// 监听 show 变化，当选择器打开时重新格式化数据
			show(newVal) {
				if (newVal) {
					// 更新 indexPop 为最新的 indexs
					this.indexPop = [...this.indexs]
					// 重新格式化数据，确保回显正确
					this.formatData()
				}
			...
			indexs: {
				handler(newVal) {
					if (newVal && newVal.length > 0) {
						// 更新 indexPop
						this.indexPop = [...newVal]
						// 如果选择器已打开，更新 u-picker 的索引
						if (this.show && this.$refs.uPicker) {
							this.$refs.uPicker.setIndexs(newVal, true)
						}
					}
				},
				deep: true

	...
	<view>
				...
				<view style="position: relative; width: 100vw; height: 100vh; background: #000; overflow: hidden;"
					...
					<view style="position: relative;">
						<video v-if="videoItem.videoUrl" :id="'video_' + index" :src="videoItem.videoUrl" :controls="false" :loop="true"
							...
							@ended="onVideoEnded" @error="onVideoError" @loadedmetadata="onLoadedmetadata" @timeupdate="onTimeUpdate"
					...
					<view
						...
						<view style="margin-bottom: 40rpx; text-align: center;">
							<u-transition :show="!videoItem.isLike" mode="fade" duration="2000">
								<u-icon v-if="!videoItem.isLike" name="/static/praise_white.png" size="50"
									@click="praiseNotes(videoItem.id)">
								</u-icon>
							</u-transition>
							<u-transition :show="videoItem.isLike" mode="fade" duration="2000">
								<u-icon v-if="videoItem.isLike" name="/static/praise_select.png" size="50"
									@click="praiseNotes(videoItem.id)">
								</u-icon>
							</u-transition>
							<view style="font-size: 26rpx; color: #fff; margin-top: 5rpx;">
								{{videoItem.likeCount || '点赞'}}
							</view>
						</view>
					...
					<view style="position: absolute; bottom: 180rpx; left: 20rpx; right: 120rpx; z-index: 98;">
						...
						<rich-text :nodes="videoItem.content" style="font-size: 28rpx; color: #fff; line-height: 1.5;"
		...
		<view v-else-if="!commentShow"
			...
			<view style="position: relative;">
				...
				<!-- 加载状态 -->
				<view v-if="isLoading"
					style="position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); z-index: 3; pointer-events: none;">
					<u-loading-icon mode="circle" size="40" color="#ffffff"></u-loading-icon>
				</view>
			...
			<view
				...
				<view style="margin-bottom: 40rpx; text-align: center;">
					<u-transition :show="!notesDetail.isLike" mode="fade" duration="2000">
						<u-icon v-if="!notesDetail.isLike" name="/static/praise_white.png" size="50"
							@click="praiseNotes(notesDetail.id)">
						</u-icon>
					</u-transition>
					<u-transition :show="notesDetail.isLike" mode="fade" duration="2000">
						<u-icon v-if="notesDetail.isLike" name="/static/praise_select.png" size="50"
							@click="praiseNotes(notesDetail.id)">
						</u-icon>
					</u-transition>
					<view style="font-size: 26rpx; color: #fff; margin-top: 5rpx;">
						{{notesDetail.likeCount || '点赞'}}
					</view>
				</view>
	...
	export default {
		onShareAppMessage() {
			...
			return {
				...
				path: `/pkg-detail/pages/notesDetail/noteVideoD?notesId=${this.notesDetail?.id || ''}`,
				imageUrl: this.notesDetail?.coverPicture || '/static/logoImage.png'
			}
		},
		onShareTimeline() {
			this.reportShareBehavior();
		...
		data() {
			return {
				...
				isLoading: false, // 添加加载状态，默认不显示加载
				lastTapTime: 0,
				progress: 0,
				currentTime: '00:00',
				totalTime: '00:00',
		...
		methods: {
			...
			togglePlay() {
				...
				// 如果是视频分类模式，使用当前索引对应的video context
			 ...
			 */
			praiseNotes(id) {
				const targetVideo = this.fromVideoCategory 
					? this.videoList.find(v => v.id === id) 
					: this.notesDetail;
						...
						if (this.fromVideoCategory) {
							...
							if (index !== -1) {
								if (this.videoList[index].isLike) {
									this.videoList[index].likeCount = this.videoList[index].likeCount - 1;
								...
								if (index === this.currentVideoIndex) {
									this.notesDetail = this.videoList[index];
								}
							}
						} else {
							// 非视频分类模式
				...
				collectOrCancelNotes({
					...
					if (res.code == 200) {
						if (this.fromVideoCategory) {
							...
							const index = this.videoList.findIndex(v => v.id === id);
							if (index !== -1) {
								if (this.videoList[index].isCollect) {
									this.videoList[index].collectionCount = this.videoList[index].collectionCount - 1;
									this.videoList[index].isCollect = false;
								} else {
									this.videoList[index].collectionCount = this.videoList[index].collectionCount + 1;
									this.videoList[index].isCollect = true;
								}
								// 同步更新当前显示的详情
								if (index === this.currentVideoIndex) {
									this.notesDetail = this.videoList[index];
			...
			async getFirstComment(notesId) {
				...
				getCommentFirstListByNotesId({
					...
					// 处理图片信息
					const processedArr = await Promise.all(mappedArr.map((item) => {
			...
			onVideoSwiperChange(e) {
				...
				if (this.videoList[newIndex]) {
					this.notesDetail = this.videoList[newIndex];
					this.commentCount = this.videoList[newIndex].commentCount || 0;
					// 重新加载评论
					this.commentList = [];
					this.page = 1;
					this.status = 'loadmore';
					this.getFirstComment(this.videoList[newIndex].id);
				}
			 ...
			 * 加载视频列表
			 */
			async loadVideoList(currentNotesId) {
				try {
					...
					if (res.code === 200 && res.data && res.data.records) {
						...
						const mappedVideos = res.data.records.map(item => {
							...
							if (item.urls) {
								...
								if (Array.isArray(item.urls)) {
									...
									try {
										...
										urlsArr = Array.isArray(parsed) ? parsed : [];
									} catch (e) {
										// 兜底：逗号分隔字符串
										urlsArr = item.urls
											.split(',')
							...
							return {
								id: item.id || item.nid || '',
								title: item.title || '',
								content: parseHtml('<p>' + item.title + '    ' + (item.content ? item.content.substring(3) : '')),
						...
						// 找到当前视频的索引
						if (currentNotesId && this.currentVideoIndex === 0) {
							const index = this.videoList.findIndex(v => String(v.id) === String(currentNotesId));
							if (index !== -1) {
								this.currentVideoIndex = index;
								console.log('✅ 找到当前视频索引:', index, '视频ID:', currentNotesId);
							}
						}
			 ...
			 * 加载当前视频详情
			 */
			loadCurrentVideoDetail(notesId) {
				// 添加浏览记录
				...
				getNotesByNotesId({
					...
					// 兼容两种返回结构：{ code, data, msg } 或 { data: { code, data, msg } }
					const data = res && res.data && Object.prototype.hasOwnProperty.call(res.data, 'data')
						? res.data.data
						: res.data;
					...
					// 处理时间
					const createTime = data.createTime ? weChatTimeFormat(data.createTime) : '';
					const updateTime = data.updateTime ? weChatTimeFormat(data.updateTime) : '';
					// 统一映射
					const videoDetail = {
						id: data.id,
						title: data.title,
					...
					console.log('笔记视频详情页 - 评论数量:', this.commentCount, '原始数据:', data.commentCount)
					
					// 加载评论
					this.getFirstComment(notesId);
		...
		onLoad(options) {
			...
			if (options.fromVideoCategory === '1') {
				...
				// 先加载视频列表
				this.loadVideoList(options.notesId).then(() => {
					// 视频列表加载完成后，再加载当前视频详情
					this.loadCurrentVideoDetail(options.notesId);
				});
			} else {
				// 非视频分类模式，使用原有逻辑
				this.loadCurrentVideoDetail(options.notesId);
		...
		onUnload() {
			if (this.detailOpenTime > 0 && this.notesDetail && this.notesDetail.id) {
				...
				reportNoteBehavior({
					...
					nid: String(this.notesDetail.id),
					durationSec,
					completed: this.videoCompleted ? 1 : 0
				});
			}
		}
	}
...
<style lang="scss">
	...
	/* 视频分类模式下进度条的位置调整 */
	.custom-progress-in-swiper {
		bottom: 120rpx !important; /* 调整进度条位置，在底部信息上方 */
		z-index: 99 !important; /* 确保进度条在底部信息之上 */
	}
</style>
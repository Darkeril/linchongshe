<template>
  <div class="container">
    <Breadcrumb :items="['IM管理', '消息管理', '群消息']" />
    <a-card
...
import {
  getImGroupMessageList,
  getImGroupMessageInfo,
} from '@/api/im/groupMessage';
import type { ImGroupMessageVo } from '@/api/im/groupMessage';
...
const columns: TableColumnData[] = [
  ...
  { title: '群名', dataIndex: 'groupName', width: 100, align: 'center' },
  {
    title: '发送用户',
...
const messageList = ref<ImGroupMessageVo[]>([]);
...
const detailRecord = ref<ImGroupMessageVo | null>(null);
...
const fetchList = async () => {
  setLoading(true);
  try {
    const res = (await getImGroupMessageList({
      content: queryParams.content || undefined,
      pageNum: queryParams.pageNum,
      pageSize: queryParams.pageSize,
    })) as {
      rows?: ImGroupMessageVo[];
      total?: number;
      data?: { rows?: ImGroupMessageVo[]; total?: number };
    };
...
const handleDetail = async (record: ImGroupMessageVo) => {
  try {
    const res = (await getImGroupMessageInfo(record.id)) as {
      data?: ImGroupMessageVo;
    };

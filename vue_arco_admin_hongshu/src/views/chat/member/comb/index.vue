<template>
  <div class="container">
    <Breadcrumb
      :items="[$t('menu.chat'), $t('menu.chat.member.comb')]"
    ></Breadcrumb>
    <a-card
      ...
      <a-row>
        <a-col :flex="1">
          ...
          >
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="name" :label="$t('biz.planName')">
                  <a-input v-model="query.name" :placeholder="$t('biz.pleaseInputPlanName')" allow-clear @press-enter="handleQuery" />
                </a-form-item>
              </a-col>
            </a-row>
          </a-form>
...
import { pageComb, delComb } from '@/api/chat/comb'
...
function handleBatchDelete() {
  ...
  Promise.all(selectedRowKeys.value.map(id => delComb(id))).then(() => {
    Message.success(t('biz.deleteSuccess'))
...
const columns = computed(() => [
  ...
  { title: t('biz.planName'), dataIndex: 'name', align: 'center' },
  { title: t('biz.planPrice'), dataIndex: 'price', align: 'center' },
  { title: t('biz.planPower'), dataIndex: 'num', align: 'center' },

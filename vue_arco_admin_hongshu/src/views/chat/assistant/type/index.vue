<template>
  <div class="container">
    <Breadcrumb
      :items="[$t('menu.chat'), $t('menu.chat.assistant.type')]"
    ></Breadcrumb>
    <a-card
      ...
      <a-row>
        <a-col :flex="1">
          ...
          >
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="name" :label="$t('biz.categoryName')">
                  <a-input v-model="query.name" :placeholder="$t('biz.pleaseInputCategoryName')" allow-clear @press-enter="handleQuery" />
                </a-form-item>
              </a-col>
            </a-row>
          </a-form>
...
import { pageAssistantType, delAssistantType } from '@/api/chat/assistant-type'
...
function fetchList() {
  ...
  pageAssistantType(params).then((res: any) => {
...
function handleBatchDelete() {
  ...
  Promise.all(selectedRowKeys.value.map(id => delAssistantType(id))).then(() => {
    Message.success(t('biz.deleteSuccess'))
...
const columns = computed(() => [
  ...
  { title: t('biz.categoryName'), dataIndex: 'name', align: 'center' },

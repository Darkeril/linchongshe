<template>
  <div class="container">
          ...
          <a-space>
            <a-button-group>
              <a-button
                ...
                type="text"
                size="mini"
                @click="handleUpdate(record.jobId)"
              >
                <template #icon>
                  <icon-edit />
                </template>
    ...
    <view-job v-model:id="state.currentId" v-model:visible="state.viewVisible">
    </view-job>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const handleStatusChange = (record: JobRecord) => {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk: () => {
        changeJobStatus(record.jobId as number, value)
          .then(() => {

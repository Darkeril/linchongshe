<template>
  <div class="container">
    <!--面包屑-->
    <Breadcrumb
      :items="[$t('menu.web.note'), $t('menu.web.xiaohongshu')]"
    ></Breadcrumb>

    <!-- 免责声明 -->
    <a-alert type="warning" style="margin-bottom: 20px" :closable="true">
      <template #title>{{ $t('biz.spiderNoticeTitle') }}</template>
      <div>
        <p>{{ $t('biz.spiderNoticeLine1') }}</p>
        <p>{{ $t('biz.spiderNoticeLine2') }}</p>
        <p>{{ $t('biz.spiderNoticeLine3') }}</p>
        <p>{{ $t('biz.spiderNoticeLine4') }}</p>
      </div>
    </a-alert>
    ...
    >
        ...
        <a-col :flex="1">
          ...
          >
            <a-row :gutter="[32, 16]" align="end">
              <a-col :span="7">
                <a-form-item field="keyword" :label="$t('biz.spiderKeyword')">
                  <a-input
                    v-model="state.queryParams.keyword"
                    :placeholder="$t('biz.spiderKeywordPlaceholder')"
                    allow-clear
              ...
              <a-col :span="4">
                <a-form-item field="type" :label="$t('biz.type')">
                  <a-select
                    v-model="state.queryParams.type"
                    :placeholder="$t('biz.selectTypePlaceholder')"
                  >
                    <a-option value="note">{{ $t('biz.noteTypeShort') }}</a-option>
                    <a-option value="image">{{ $t('biz.imageShort') }}</a-option>
                    <a-option value="video">{{ $t('biz.video') }}</a-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col :span="4">
                <a-form-item field="sortType" :label="$t('biz.sortTypeLabel')">
                  <a-select
                    v-model="state.queryParams.sortType"
                    :placeholder="$t('biz.selectSortPlaceholder')"
                  >
                    <a-option :value="0">{{ $t('biz.sortComposite') }}</a-option>
                    <a-option :value="1">{{ $t('biz.sortLatest') }}</a-option>
                    <a-option :value="2">{{ $t('biz.sortMostLike') }}</a-option>
                    <a-option :value="3">{{ $t('biz.sortMostComment') }}</a-option>
                    <a-option :value="4">{{ $t('biz.sortMostCollect') }}</a-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col :span="4">
                <a-form-item field="noteTime" :label="$t('biz.noteTimeLabel')">
                  <a-select
                    v-model="state.queryParams.noteTime"
                    :placeholder="$t('biz.selectNoteTimePlaceholder')"
                  >
                    <a-option :value="0">{{ $t('biz.noteTimeUnlimited') }}</a-option>
                    <a-option :value="1">{{ $t('biz.noteTimeOneDay') }}</a-option>
                    <a-option :value="2">{{ $t('biz.noteTimeOneWeek') }}</a-option>
                    <a-option :value="3">{{ $t('biz.noteTimeHalfYear') }}</a-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col :span="5">
            ...
            <a-row :gutter="16" style="margin-top: 16px">
              <a-col :span="24">
                <a-form-item field="cookie" :label="$t('biz.cookieOptionalLabel')">
                  <a-textarea
                    v-model="state.queryParams.cookie"
                    :placeholder="$t('biz.cookiePlaceholder')"
                    :allow-clear="true"
      ...
      <a-empty
        v-else
        :description="$t('biz.emptySpiderHint')"
        style="margin-top: 40px"
      />
    </a-card>
    ...
    >
      <div v-if="state.currentItem" class="detail-content">
          ...
          <div class="video-wrapper-compact">
            ...
            >
              <div class="video-downloading-content">
                ...
                <p class="loading-text-primary">
                  {{ $t('biz.downloadingVideo') }}
                </p>
                <p class="loading-text-secondary">
                  {{ $t('biz.videoDownloadWait') }}
                </p>
              </div>
            </div>
            <!-- 视频加载中提示 -->
            <div v-else-if="state.videoLoading" class="video-loading-tip">
              <a-spin size="large" :tip="$t('biz.loadingVideoTip')">
            ...
            <div v-else class="video-placeholder">
              <div style="text-align: center; padding: 40px 20px">
                ...
                >
                  {{ $t('biz.videoDownloadedLoading') }}
                </p>
                <p v-else style="color: #86909c; margin-bottom: 20px">{{
                  $t('biz.videoNotLoaded')
                }}</p>
        ...
        <a-card
          ...
          :title="
            state.currentItem.type === 'video'
              ? $t('biz.videoCoverTitle')
              : $t('biz.imageListTitle', {
                  n: state.currentItem.images.length,
                })
          "
        >
          <a-row :gutter="[6, 6]">
        ...
        <a-card
          :bordered="false"
          class="meta-card-compact"
          :title="$t('biz.otherInfoTitle')"
        >
          <a-descriptions :column="2" :size="small" bordered>
            <a-descriptions-item :label="$t('biz.noteIdLabel')">
              <span class="meta-value-compact">{{ state.currentItem.id }}</span>
            </a-descriptions-item>
            <a-descriptions-item
              v-if="state.currentItem.publishTime"
              :label="$t('biz.publishTimeLabel')"
            >
              <span class="meta-value-compact">{{
                state.currentItem.publishTime
              }}</span>
            </a-descriptions-item>
            <a-descriptions-item :label="$t('biz.linkLabel')" :span="2">
              <a
...
const handleSearch = async (isPageChange = false) => {
  if (!state.queryParams.keyword.trim()) {
    Message.error(t('biz.enterKeywordError'));
    return;
  }
  ...
  try {
    ...
    if (responseCode === 'success' || responseCode === '200') {
      ...
      const newItems: XiaohongshuItem[] = Array.isArray(responseData) ? responseData : [];
      ...
      if (isPageChange) {
        ...
        Message.success(t('biz.spiderSuccessCount', { n: newItems.length }));
      }
      
      // 标记当前页已加载
      state.loadedPages.add(state.currentPage);
      
      // 更新总数：如果当前页返回的数据量等于pageSize，说明可能还有更多数据
      // 设置total为当前页数 * pageSize + pageSize，这样分页组件会显示"还有下一页"
      // 如果返回的数据量小于pageSize，说明已经是最后一页，使用实际数据条数
      if (newItems.length >= state.pageSize) {
        // 可能还有更多数据，设置一个估算值让分页组件显示"还有下一页"
        state.total = state.currentPage * state.pageSize + state.pageSize;
      } else {
        // 已经是最后一页，使用实际数据条数
        state.total = state.items.length;
      }
    } else {
      const errorMsg = (response as any).message || t('biz.spiderFailed');
    ...
    Message.error(error.message || t('biz.spiderNetworkError'));
    if (!isPageChange) {
      state.items = [];
      state.total = 0;
    }
...
const handleDownloadVideo = async () => {
  if (!state.currentItem?.video || !state.currentItem.video.trim()) {
    Message.error(t('biz.videoUrlEmpty'));
    return;
  }
  ...
  if (isOssUrl(state.currentItem.video)) {
    ...
    Message.info(t('biz.videoUseOss'));
    return;
  }
  ...
  try {
    ...
    if (response.code === 'success' || response.code === 200) {
      ...
      Message.success(t('biz.videoDownloadSuccessLoading'));
      ...
      throw new Error(response.message || t('biz.downloadFailed'));
    }
  } catch (error: any) {
    console.error('下载视频失败:', error);
    state.videoLoadError = true;
    Message.error(
      t('biz.downloadVideoFailedPrefix', {
...
const handleVideoError = (event: Event) => {
  ...
  if (error) {
    ...
    // 根据错误代码显示相应的错误信息
    if (error.code === 2) {
      Message.error(t('biz.videoLoadErrorNetwork'));
    } else if (error.code === 3) {
      Message.error(t('biz.videoLoadErrorDecode'));
    }
    // 错误代码4（格式不支持）和其他错误不显示提示
  } else {
    Message.warning(t('biz.videoCannotPlay'));
  }
...
const handleCopyUrl = async (url: string) => {
  try {
    ...
    Message.success(t('biz.linkCopied'));
  } catch (error) {
    Message.error(t('biz.copyFailed'));
  }
};
...
const handlePublish = async (item: XiaohongshuItem) => {
  try {
    ...
    const initialData: any = {
      title: item.title || t('biz.noTitle'),
    ...
    if (item.type === 'video' && item.video) {
      ...
      if (item.images.length > maxImages) {
        Message.warning(t('biz.imageCountExceeded', { n: maxImages }));
      }
    }
    ...
    Message.info(t('biz.noteDataFilledHint'));
  } catch (error) {
    console.error('发布失败:', error);
    Message.error(t('biz.openPublishFailed'));
  }
};
...
const formatCount = (count: number): string => {
  if (count >= 10000) {
    return `${(count / 10000).toFixed(1)}${t('biz.countTenThousand')}`;
  }
  if (count >= 1000) {
...
.container {
  padding: 0 40px 20px 40px; // 增加左右边距从20px到40px
}

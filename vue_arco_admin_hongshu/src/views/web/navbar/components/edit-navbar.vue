...
<script lang="ts" setup>
  ...
  const beforeUpload = (file: File) => {
    ...
    // 立即保存文件对象到 state.form.file（与会员管理页面保持一致）
    state.form.file = file;
    console.log('beforeUpload: 文件已保存到 state.form.file');
    console.log('state.form.file:', state.form.file);
    console.log('state.form.file instanceof File:', state.form.file instanceof File);
    ...
    reader.onload = (e) => {
      ...
      console.log('beforeUpload: fileList 已更新:', fileList.value);
      console.log('beforeUpload: state.form.file 仍然存在:', state.form.file);
    };
    reader.onerror = (error) => {
      console.error('beforeUpload: FileReader 读取失败:', error);
    };
    reader.readAsDataURL(file);
    
    console.log('========== beforeUpload 结束 ==========');
  ...
  const handleChange = (fileListParam: any, fileItem: any) => {
    ...
    console.log('当前 state.form.file (handleChange 开始时):', state.form.file);
    console.log('当前 fileList.value:', fileList.value);
    
    // 更新 fileList
    if (Array.isArray(fileListParam)) {
      fileList.value = fileListParam;
      console.log('fileList 已更新为数组:', fileList.value);
    } else if (fileListParam) {
      fileList.value = [fileListParam];
      console.log('fileList 已更新为单个文件:', fileList.value);
    } else {
      fileList.value = [];
    ...
    // 在 picture-card 模式下，fileListParam 可能是单个文件对象
    let file: any = null;
    ...
    if (file) {
      console.log('file 对象的 keys:', Object.keys(file));
    ...
    if (file) {
      ...
      if (fileObj) {
        ...
        console.log('handleChange: 文件对象已保存到 state.form.file');
        ...
        if (!state.form.normalCover || !state.form.normalCover.startsWith('data:')) {
          console.log('开始读取文件预览');
          const reader = new FileReader();
          reader.onload = (e) => {
            const previewUrl = e.target?.result as string;
            state.form.normalCover = previewUrl;
            console.log('文件预览已读取:', `${previewUrl.substring(0, 50)}...`);
            ...
            if (fileList.value.length > 0) {
              ...
              console.log('fileList 已更新预览 URL');
            }
          };
          reader.onerror = (error) => {
        ...
        console.warn('无法获取文件对象');
        console.warn('file 结构:', file);
        console.warn('fileItem 结构:', fileItem);
        // 注意：不要清空 state.form.file，因为可能之前已经保存了文件对象
        if (!state.form.file) {
      ...
      state.form.file = undefined;
      console.log('handleChange: 没有文件，且之前也没有文件对象，已清空');
    } else {
      // 没有文件，但之前已有文件对象，保留文件对象
      console.log('handleChange: 没有文件，但之前已有文件对象，保留文件对象');
    }
    
    console.log('handleChange 结束, state.form.file:', state.form.file);
    console.log('========== handleChange 结束 ==========');
  };
  ...
  const handleConfirm = () => {
    formRef.value?.validate().then((validateError: any) => {
      if (!validateError) {
        ...
        console.log('========== 提交时的检查 ==========');
        console.log('state.form.file:', state.form.file);
        console.log('state.form.file 类型:', typeof state.form.file);
        console.log('state.form.file instanceof File:', state.form.file instanceof File);
        ...
        if (fileToUpload) {
          ...
          console.log('文件已添加到 FormData, 文件名:', fileToUpload.name, '文件大小:', fileToUpload.size);
        } else {
          console.log('没有文件对象，跳过文件上传');
          console.log('说明：如果这是修改已有导航栏且没有更换封面图，这是正常的');
        }
        console.log('========== 提交检查结束 ==========');
        ...
        if (state.form.id !== undefined) {
          // 修改导航栏
          updateNavbar(params as any).then(() => {
  ...
  const getNavbarForm = async (id: number) => {
    ...
    const { data, code, msg } = await getNavbar(id);
      ...
      const { data: navbarList } = await listNavbar({});
    // 将列表数据转换为树结构并添加“主类目”

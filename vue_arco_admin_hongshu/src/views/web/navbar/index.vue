<template>
  <div class="container">
    <!--面包屑-->
    <Breadcrumb :items="['门户管理', '导航栏管理']"></Breadcrumb>
    <a-card
...
const toNoteList = (row: any) => {
  router.push({ path: '/note/note', query: { pid: row.id } });
};
...
const handleStatusChange = (record: any, nextStatus: string) => {
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk: () =>
      updateNavbarStatus(record.id, nextStatus).then(() => {
...
const fetchNavbarData = async () => {
  ...
  state.navbarList = listToTree(data);
  setLoading(false);
};
fetchNavbarData();
...
// 删除菜单
const handleDelete = (row: NavbarRecord) => {
  Modal.confirm({
    titleAlign: 'start',
    title: '系统提示',
    ...
    cancelText: '取消',
    onOk: () => {
      delNavbar(row.id as number).then(() => {

...
| **侧边抽屉** | `components/DrawerMenu.vue`（新建） | `screens-part12.jsx` DrawerScreen | 从"我的"页面左滑呼出，含用户卡片+宠物快切+三组菜单+夜间模式 |
...
- **侧边抽屉（新增）** → 左滑呼出，包含：
  - 顶部渐变用户卡片（头像+昵称+等级+统计数据）
  - 宠物快切横排（喵酱/球球/闪闪 + 添加按钮）
  - 三组菜单：我的账户（钱包/券/订单/收藏）、激励成长（签到/会员/邀请/成就）、工具帮助（历史/草稿/客服/设置/关于）
  - 夜间模式开关
  - 底部版本号+退出登录
- 设置页 → 分组卡片式列表
- 编辑资料 → 圆角表单输入框

...
class NativeMsg {
	...
	create() {
		this.loadBg().then(() => {
			...
			_view.drawBitmap(
				this.bitmap, {}, {
					width: this.viewStyle.width,
					height: this.viewStyle.height,
					left: 0,
					top: 0
				},
				"alarm-bg"
...
// 对外暴露一个创建实例的方法
export function createAlarm(item, res) {
	// 如果存在当前的 NativeMsg 实例，则先关闭它
	if (currentAlarm) {
		currentAlarm.hide();
		currentAlarm = null;
	}
	...
	// 创建新的 NativeMsg 实例
	const newAlarm = new NativeMsg(item, res);
	...
	setTimeout(() => {
		if (currentAlarm === newAlarm) {
			// 只有当前实例是新创建的实例时才进行隐藏操作
			newAlarm.animationHide()
			currentAlarm = null;
		}
	}, 2000);
	return newAlarm;

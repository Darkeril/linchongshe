package com.hongshu.common.core.excel.core;
...
import com.hongshu.common.core.excel.annotation.ExcelDictFormat;
import com.hongshu.common.core.excel.annotation.ExcelEnumFormat;
import com.hongshu.common.core.exception.ServiceException;
import com.hongshu.common.core.utils.ImStringUtils;
import com.hongshu.common.core.utils.SpringUtils;
import com.hongshu.common.core.utils.StreamUtils;
    ...
    public void afterSheetCreate(WriteWorkbookHolder writeWorkbookHolder, WriteSheetHolder writeSheetHolder) {
        ...
        for (Map.Entry<Integer, FieldWrapper> entry : fieldCache.getSortedFieldMap().entrySet()) {
            ...
            if (field.isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                if (ImStringUtils.isNotBlank(dictType)) {
                    // 如果传递了字典名，则依据字典建立下拉
                    Collection<String> values = Optional.ofNullable(dictService.getAllDictByDictType(dictType))
                            .orElseThrow(() -> new ServiceException(String.format("字典 %s 不存在", dictType)))
                            .values();
                    options = new ArrayList<>(values);
                } else if (ImStringUtils.isNotBlank(converterExp)) {
                    // 如果指定了确切的值，则直接解析确切的值
                    List<String> strList = ImStringUtils.splitList(converterExp, format.separator());
                    options = StreamUtils.toList(strList, s -> ImStringUtils.split(s, "=")[1]);
                }

package com.hongshu.ai.api.internlm;
...
import com.hongshu.ai.api.internlm.constant.ApiConstant;
import com.hongshu.ai.api.internlm.entity.ModelsList;
import com.hongshu.ai.api.internlm.entity.request.ChatCompletion;
import com.hongshu.ai.api.internlm.entity.response.ChatResponse;
import com.hongshu.ai.api.internlm.interceptor.InternlmInterceptor;
import com.hongshu.ai.api.internlm.interceptor.InternlmLogger;
import com.hongshu.ai.api.internlm.listener.SSEListener;
...
public class InternlmClient implements KeyUpdater {
    ...
    public Boolean streamChat(HttpServletResponse response, ChatCompletion chat, Long chatId, String parentMessageId, String version, String uid, Boolean isWs) {
        chat.stream = true;
        try {
            String requestBody = new ObjectMapper().writeValueAsString(chat);
            log.info("书生浦语请求参数: {}", requestBody);
            Request request = new Request.Builder().url(ApiConstant.CHAT_COMPLETION_URL)
                    .addHeader(AuthConstant.JWT_TOKEN_HEADER, AuthConstant.JWT_TOKEN_PREFIX + this.apiKey)
            ...
            log.error("书生浦语流式聊天异常: {}", e.getMessage(), e);
            return true;
        }
    }

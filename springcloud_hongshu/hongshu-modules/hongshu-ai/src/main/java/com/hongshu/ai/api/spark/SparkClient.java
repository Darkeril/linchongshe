package com.hongshu.ai.api.spark;
...
import com.hongshu.ai.api.spark.listener.SyncListener;
...
public class SparkClient implements KeyUpdater {
    ...
    public SparkClient(Builder builder) {
        ...
        if (StrUtil.isBlank(builder.appSecret)) {
            throw new ValidateException("构造错误: apiSecret不能为空");
        }
    ...
    private Request getRequest(ModelEnum model) throws Exception {
        ...
        if (StrUtil.isBlank(apiSecret)) {
            throw new Exception("apiSecret不能为空");
        }

...
public class BaiduQianfanAuditServiceImpl implements ContentAuditService {
    ...
    public AutoAuditResult auditText(String content) {
        try {
            ...
            log.info("百度千帆文本审核配置 - enabled: {}, accessKey: {}", config.getEnabled(), config.getAccessKey());
    ...
    private AutoAuditResult auditImage(String imageUrl) {
        try {
            BaiduQianfanConfigDTO config = systemConfigService.getBaiduQianfanConfigInternal();
            log.info("百度千帆图片审核配置 - enabled: {}, imageUrl: {}", config.getEnabled(), imageUrl);
            if (!config.getEnabled()) {
                return AutoAuditResult.builder()
                    .result(AutoAuditResultEnum.MANUAL)
                    .reason("百度千帆审核未开启")
     ...
     * 调用百度千帆文本审核API
     */
    private TextAuditResponse callBaiduTextAudit(String content, BaiduQianfanConfigDTO config) {
        TextAuditResponse response = new TextAuditResponse();
        try {
            if (config.getTextEndpoint() == null || config.getTextEndpoint().isEmpty()) {
            ...
            String accessToken = getAccessToken(config.getAccessKey(), config.getSecretKey());
            if (accessToken == null) {
                // token 获取失败，返回错误响应，应该被解析为 ERROR 而不是 MANUAL
                response.setConclusion("审核服务异常");
                response.setConfidenceScore(0.0);
                response.setLogId("get_token_failed");
                return response;
            }

            String requestUrl = config.getTextEndpoint() + "?access_token=" + accessToken;
     ...
     * 调用百度千帆图片审核API
     */
    private ImageAuditResponse callBaiduImageAudit(String imageUrl, BaiduQianfanConfigDTO config) {
        ImageAuditResponse response = new ImageAuditResponse();
        try {
            if (config.getImageEndpoint() == null || config.getImageEndpoint().isEmpty()) {
                response.setConclusion("疑似");
                response.setConfidenceScore(0.0);
            ...
            String requestUrl = config.getImageEndpoint() + "?access_token=" + accessToken;
    ...
    private AutoAuditResult parseTextAuditResponse(TextAuditResponse response, Double threshold) {
        ...
        String reason = response.getConclusion();
        
        // 检查是否是 token 获取失败或其他系统错误
        if ("get_token_failed".equals(response.getLogId()) || 
            "审核服务异常".equals(response.getConclusion()) ||
            "no_text_endpoint".equals(response.getLogId())) {
            result = AutoAuditResultEnum.ERROR;
            reason = "百度千帆审核服务异常：无法获取 access_token 或配置错误";
        } else if ("合规".equals(response.getConclusion())) {
            result = AutoAuditResultEnum.PASS;
     ...
     * 调用百度千帆视频审核API
     */
    private VideoAuditResponse callBaiduVideoAudit(String videoUrl, String textContext, BaiduQianfanConfigDTO config) {
        VideoAuditResponse response = new VideoAuditResponse();
        try {
            if (config.getVideoEndpoint() == null || config.getVideoEndpoint().isEmpty()) {
            ...
            // 2. 组装请求URL
            String requestUrl = config.getVideoEndpoint() + "?access_token=" + accessToken;
            ...
            try {
                ...
                conn.setReadTimeout(60000);     // 读取超时：60秒（视频审核需要更长时间，因为需要处理视频内容）
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8");
    ...
    private String getAccessToken(String ak, String sk) throws IOException {
        ...
        try {
            ...
            if (resultJson == null || resultJson.isEmpty()) {
                log.error("获取百度千帆 access_token 失败：响应为空");
                return null;
            }
            ...
            if (obj.containsKey("error")) {
                ...
                log.error("获取百度千帆 access_token 失败 - error: {}, error_description: {}", error, errorDescription);
                return null;
            }
            ...
            if (accessToken == null || accessToken.isEmpty()) {
                log.error("获取百度千帆 access_token 失败：响应中未包含 access_token 字段");
                return null;
            }
            
            return accessToken;
        } catch (Exception e) {
            log.error("获取百度千帆 access_token 异常", e);
            return null;
        } finally {
            if (conn != null) {
                conn.disconnect();
    ...
    private AutoAuditResult parseImageAuditResponse(ImageAuditResponse response, Double threshold) {
        ...
        if ("get_token_failed".equals(response.getLogId()) || 
            "审核服务异常".equals(response.getConclusion()) ||
            "no_image_endpoint".equals(response.getLogId()) ||
            "empty_response".equals(response.getLogId())) {
            result = AutoAuditResultEnum.ERROR;
            reason = "百度千帆审核服务异常：无法获取 access_token 或配置错误";
        } else if ("合规".equals(response.getConclusion())) {
            result = AutoAuditResultEnum.PASS;

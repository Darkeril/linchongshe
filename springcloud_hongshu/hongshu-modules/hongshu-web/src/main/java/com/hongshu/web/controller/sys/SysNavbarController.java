...
public class SysNavbarController extends BaseController {
    ...
    @PostMapping
    public AjaxResult add(@RequestParam("category") String category, @RequestParam(value = "file", required = false) MultipartFile file) {
...
//        category.setCreator(getUsername());
    ...
    @PutMapping("/update")
    public AjaxResult edit(@RequestParam("category") String category,
...
//        category.setUpdater(getUsername());
    ...
    public AjaxResult updateStatus(@RequestParam Long id, @RequestParam String status) {
        navbarService.updateStatus(id, status);
